...
from ..services.video import upload_video as save_video_db
...
async def create_video(
    ...
    return await save_video_db(db, video_data)
...
async def read_video(video_id: int, db: AsyncSession = Depends(get_db)):
    return await save_video_db.get_video(db, video_id)
...
async def read_by_channel(channel_id: int, db: AsyncSession = Depends(get_db)):
    return await save_video_db.list_videos_by_channel(db, channel_id)
